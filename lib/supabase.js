...
export function isDemoCategory(cat) {
  if (!cat) return false;
  const id = String(cat.id || "").toLowerCase();
  const slug = String(cat.slug || "").toLowerCase();
  const name = String(cat.name || "").toLowerCase();
  if (id === "cat-1" || id === "cat-2" || id === "cat-3" || id.startsWith("cat-demo-") || id.startsWith("demo-")) return true;
  if (["skin-care", "hair-care", "pooja"].includes(slug) && ["cat-1", "cat-2", "cat-3"].includes(id)) return true;
  return false;
}
...
const LOCAL_STORAGE_DELETED_CATEGORIES_KEY = "devora_deleted_categories";

export function getDeletedCategoryIds() {
  if (typeof window === "undefined") return [];
  const stored = localStorage.getItem(LOCAL_STORAGE_DELETED_CATEGORIES_KEY);
  if (!stored) return [];
  try {
...
export function addDeletedCategoryId(id) {
  if (typeof window !== "undefined" && id) {
    try {
      ...
      if (!list.includes(strId)) {
        list.push(strId);
        localStorage.setItem(LOCAL_STORAGE_DELETED_CATEGORIES_KEY, JSON.stringify(list));
      }
    } catch (_) {}
  }
}

export function isDemoContactEmail(email) {
  if (!email) return false;
  const e = String(email).trim().toLowerCase();
  return e.endsWith("@example.com");
}

export function isDemoContactPhone(phone) {
  if (!phone) return false;
  const p = String(phone).replace(/[^0-9]/g, "");
  return p === "919876543210" || p === "9876543210";
}

export function isDemoContactAddress(address) {
  if (!address) return false;
  const a = String(address).toLowerCase();
  return a.includes("123 herbal way");
}

// Automatic cleanup on startup to purge legacy demo data from client localStorage
if (typeof window !== "undefined") {
  try {
    const rawProds = localStorage.getItem("devora_mock_products_v1");
    if (rawProds) {
      const parsed = JSON.parse(rawProds);
      if (Array.isArray(parsed)) {
        const cleaned = parsed.filter(p => !isDemoProduct(p));
        if (cleaned.length !== parsed.length) localStorage.setItem("devora_mock_products_v1", JSON.stringify(cleaned));
      }
    }
    const rawCats = localStorage.getItem("devora_mock_categories_v1");
    if (rawCats) {
      const parsed = JSON.parse(rawCats);
      if (Array.isArray(parsed)) {
        const cleaned = parsed.filter(c => !isDemoCategory(c));
        if (cleaned.length !== parsed.length) localStorage.setItem("devora_mock_categories_v1", JSON.stringify(cleaned));
      }
    }
    ...
    if (rawSettings) {
      ...
      let changed = false;
      if (isDemoContactEmail(parsed.email)) { parsed.email = ""; changed = true; }
      if (isDemoContactPhone(parsed.phone)) { parsed.phone = ""; changed = true; }
      if (isDemoContactAddress(parsed.address)) { parsed.address = ""; changed = true; }
      if (changed) {
  ...
  try {
    ...
    if (catStr) {
      ...
      if (Array.isArray(cats)) {
        const cleaned = cats.filter(c => !isDemoCategory(c));
...
// (isDemoProduct definition moved to top of file)
...
function getLocalCategories() {
  ...
  try {
    ...
    return list.filter(c => !isDemoCategory(c) && !deletedIds.includes(String(c.id)));
  } catch (e) {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        if (!error && data && Array.isArray(data)) {
          const demoFound = data.filter(p => isDemoProduct(p));
          if (demoFound.length > 0) {
            const demoIds = demoFound.map(p => p.id);
            supabase.from("products").delete().in("id", demoIds).then(() => {}).catch(() => {});
          }
  ...
  categoriesPromise = (async () => {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        if (!error && data && Array.isArray(data)) {
          remoteCategories = data.filter(c => c && !isDemoCategory(c) && !deletedIds.includes(String(c.id)));
          fetchedRemote = true;
        }
      } catch (err) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (json?.store?.categories && Array.isArray(json.store.categories)) {
            remoteCategories = json.store.categories.filter(c => c && !isDemoCategory(c) && !deletedIds.includes(String(c.id)));
            fetchedRemote = true;
          }
        }
      } catch (_) {}
    ...
    const localCategories = getLocalCategories().filter(c => c && !isDemoCategory(c) && !deletedIds.includes(String(c.id)));
...
export async function addCategory(category) {
  ...
  // If this category id or name was previously marked deleted, unmark it
  if (typeof window !== "undefined") {
    try {
      const deleted = getDeletedCategoryIds().filter(d => d !== String(localId) && d !== newCategory.slug);
      localStorage.setItem(LOCAL_STORAGE_DELETED_CATEGORIES_KEY, JSON.stringify(deleted));
    } catch (_) {}
  }
...
export async function deleteCategory(id) {
  ...
  const categories = getLocalCategories();
  const filtered = categories.filter((c) => String(c.id) !== cleanId);
  saveLocalCategories(filtered, false, true);

  // Await server-side /api/sync deletion
  if (typeof window !== "undefined") {
    try {
      await fetch("/api/sync", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "delete_category", id: cleanId }),
      });
    } catch (_) {}
  }
  ...
  if (typeof window !== "undefined") {
    ...
    window.dispatchEvent(new CustomEvent("devora_categories_updated", { detail: filtered }));
  }
  return { success: true };
}
...
export async function deleteOrder(id, authContext = {}) {
  ...
  addDeletedOrderId(cleanId);

  const orders = getLocalOrders();
  const orderToDelete = orders.find((o) => String(o.id) === cleanId || String(o.order_number || "") === cleanId);
  const updated = orders.filter((o) => String(o.id) !== cleanId && String(o.order_number || "") !== cleanId);
  ...
  if (typeof window !== "undefined") {
    try {
      await fetch("/api/sync", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "delete_order", id: cleanId }),
      });
    } catch (_) {}
  }
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      throw e; // Bubble up the error so the UI stops showing success
    }
  }
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      const [settingsRes, sfRes] = await Promise.all([
        ...
        supabase.from("storefront_settings").select("id, extended_data, updated_at").limit(1).maybeSingle(),
      ]);

      const data = settingsRes?.data;
      const sfData = sfRes?.data;
      const sfExtended = (sfData?.extended_data && typeof sfData.extended_data === "object") ? sfData.extended_data : {};
      const sfContact = (sfExtended.contact && typeof sfExtended.contact === "object") ? sfExtended.contact : {};

      if (data || sfData) {
        const locTime = local.updated_at ? new Date(local.updated_at).getTime() : 0;
        const remTime = (data?.updated_at || sfData?.updated_at) ? new Date(data?.updated_at || sfData?.updated_at).getTime() : 0;
        ...
        const dataExtended = (data?.extended_data && typeof data?.extended_data === "object") ? data.extended_data : {};

        // Clean out any demo values from remote data
        const remoteEmail = isDemoContactEmail(data?.email) ? "" : (data?.email || sfContact.email || sfExtended.email || "");
        const remotePhone = isDemoContactPhone(data?.phone) ? "" : (data?.phone || sfContact.phone || sfExtended.phone || "");
        const remoteAddress = isDemoContactAddress(data?.address) ? "" : (data?.address || sfContact.address || sfExtended.address || "");

        const localEmail = isDemoContactEmail(local.email) ? "" : (local.email || "");
        const localPhone = isDemoContactPhone(local.phone) ? "" : (local.phone || "");
        const localAddress = isDemoContactAddress(local.address) ? "" : (local.address || "");
        ...
        const merged = {
          ...
          email: remoteEmail || localEmail || DEFAULT_SETTINGS.email,
          phone: remotePhone || localPhone || DEFAULT_SETTINGS.phone,
          address: remoteAddress || localAddress || DEFAULT_SETTINGS.address,
          whatsapp: data?.whatsapp || sfContact.whatsapp || local.whatsapp || DEFAULT_SETTINGS.whatsapp || "8608540400",
          support_hours: data?.support_hours || dataExtended.support_hours || sfContact.support_hours || local.support_hours || "",
          store_name: data?.store_name || sfExtended.store_name || local.store_name || DEFAULT_SETTINGS.store_name,
          tagline: data?.tagline !== undefined ? data.tagline : (sfExtended.tagline !== undefined ? sfExtended.tagline : local.tagline || DEFAULT_SETTINGS.tagline),
          logo_url: data?.logo_url !== undefined ? data.logo_url : (sfExtended.logo_url !== undefined ? sfExtended.logo_url : (local.logo_url || "")),
          // Boolean coercions for shipping/policy flags
          shipping_enabled: data?.shipping_enabled !== undefined
            ? Boolean(data.shipping_enabled)
            : (local.shipping_enabled !== undefined ? Boolean(local.shipping_enabled) : true),
          ...
          updated_at: data?.updated_at || sfData?.updated_at || local.updated_at,
        };
...
export async function updateContactDetails(settingsData) {
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      ...
      // Base guaranteed columns in public.settings
      const basePayload = {
        updated_at: new Date().toISOString(),
      };
      if (updated.store_name !== undefined) basePayload.store_name = updated.store_name;
      if (updated.tagline !== undefined) basePayload.tagline = updated.tagline;
      if (updated.description !== undefined) basePayload.description = updated.description;
      if (updated.email !== undefined) basePayload.email = updated.email;
      if (updated.phone !== undefined) basePayload.phone = updated.phone;
      if (updated.address !== undefined) basePayload.address = updated.address;
      if (updated.whatsapp !== undefined) basePayload.whatsapp = updated.whatsapp;
      if (updated.logo_url !== undefined) basePayload.logo_url = updated.logo_url;
      if (updated.free_shipping_threshold !== undefined) basePayload.free_shipping_threshold = Number(updated.free_shipping_threshold);
      if (updated.shipping_enabled !== undefined) basePayload.shipping_enabled = Boolean(updated.shipping_enabled);
      if (updated.state_shipping_enabled !== undefined) basePayload.state_shipping_enabled = Boolean(updated.state_shipping_enabled);

      // Full payload with all optional settings columns
      const fullPayload = {
        ...basePayload,
        support_hours: updated.support_hours || "",
        order_prefix: updated.order_prefix || "DEV-",
        shipping_enabled: updated.shipping_enabled !== undefined ? Boolean(updated.shipping_enabled) : true,
        state_shipping_enabled: updated.state_shipping_enabled !== undefined ? Boolean(updated.state_shipping_enabled) : true,
        shipping_charge_tamilnadu: Number(updated.shipping_charge_tamilnadu ?? 50),
        shipping_charge_other_states: Number(updated.shipping_charge_other_states ?? 100),
        standard_shipping_charge: Number(updated.standard_shipping_charge ?? 50),
        delivery_estimate: updated.delivery_estimate || "",
        return_policy_enabled: updated.return_policy_enabled !== undefined ? Boolean(updated.return_policy_enabled) : true,
        return_window_days: Number(updated.return_window_days ?? 7),
        return_policy_text: updated.return_policy_text || "",
        instagram_url: updated.instagram_url || "",
        facebook_url: updated.facebook_url || "",
        youtube_url: updated.youtube_url || "",
        social_links_enabled: updated.social_links_enabled !== undefined ? Boolean(updated.social_links_enabled) : true,
        social_links: Array.isArray(updated.social_links) ? updated.social_links : [],
        about_badge: updated.about_badge || "",
        about_title: updated.about_title || "",
        about_description: updated.about_description || "",
        about_cards: Array.isArray(updated.about_cards) ? updated.about_cards : [],
      };
      ...
      if (existing?.id) {
        const { error: fullErr } = await supabase
          ...
          .eq("id", existing.id);
        if (fullErr) {
          console.warn("Full settings update failed, falling back to base columns:", fullErr.message);
          const { error: baseErr } = await supabase
            .from("settings")
            .update(basePayload)
            .eq("id", existing.id);
          if (baseErr) console.error("Base settings update error:", baseErr.message);
        }
      } else {
        const { error: fullInsErr } = await supabase
          .from("settings")
          .insert([fullPayload]);
        if (fullInsErr) {
          console.warn("Full settings insert failed, falling back to base columns:", fullInsErr.message);
          const { error: baseInsErr } = await supabase
            .from("settings")
            .insert([basePayload]);
          if (baseInsErr) console.error("Base settings insert error:", baseInsErr.message);
        }
      }

      // Also mirror contact details into storefront_settings.extended_data for dual persistence
      try {
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      const { data, error } = await supabase
        ...
        .order("created_at", { ascending: false });
      if (!error && data) {
        const demoFound = data.filter(c => isDemoCustomer(c));
        if (demoFound.length > 0) {
          const demoIds = demoFound.map(c => c.id);
          supabase.from("customers").delete().in("id", demoIds).then(() => {}).catch(() => {});
        }
        result = data;
        fetchedRemote = true;
      } else if (error) {
    ...
    try {
      ...
      if (existing) {
        ...
        if (error) {
          ...
          throw new Error("Supabase Storefront Update Failed: " + error.message);
        }
      } else {
        const { error } = await supabase
          .from("storefront_settings")
        ...
        if (error) {
          supabaseStorefrontUnavailableUntil = Date.now() + 60000;
          throw new Error("Supabase Storefront Insert Failed: " + error.message);
        }
      }
    } catch (err) {
      supabaseStorefrontUnavailableUntil = Date.now() + 60000;
