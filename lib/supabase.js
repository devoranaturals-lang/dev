...
export async function getCustomers() {
  ...
  if (fetchedRemote) {
    ...
    const missingLocal = localCustomers.filter(c => {
      ...
      return !idMatch && !emailMatch && !deletedIds.includes(String(c.id)) && !isDemoCustomer(c);
    });
    ...
    if (isSupabaseConfigured && supabase && now >= supabaseStorefrontUnavailableUntil) {
      try {
        ...
        if (!error && data) {
          ...
          // PostgREST 500 error or table missing: activate 2-minute circuit breaker to stop network storms
