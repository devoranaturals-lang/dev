...
export async function getProducts() {
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      ...
      if (!error && data && Array.isArray(data)) {
        // Any product retrieved from Supabase is a real database product!
  ...
  // 2. Server-side Supabase Fetch via /api/sync fallback (bypasses browser adblockers / RLS issues)
  if (!fetchedRemote || remoteProducts.length === 0) {
    try {
      const res = await fetch("/api/sync", { cache: "no-store" });
      if (res.ok) {
        const json = await res.json();
        if (json?.store?.products && Array.isArray(json.store.products) && json.store.products.length > 0) {
          const apiProds = json.store.products.filter((p) => !isDemoProduct(p));
          if (apiProds.length > 0) {
            remoteProducts = apiProds;
            fetchedRemote = true;
          }
        }
      }
  ...
  if (fetchedRemote && remoteProducts.length > 0) {
    ...
    // Background sync local-only products to Supabase if connected
    if (missingLocal.length > 0 && isSupabaseConfigured && supabase) {
      setTimeout(async () => {
        for (const lp of missingLocal) {
          try {
            const { id, ...supabasePayload } = lp;
    ...
    const merged = [...missingLocal, ...remoteProducts];
    saveLocalProducts(merged, false, false);
    return merged;
  }
...
export async function addProduct(product) {
  ...
  saveLocalProducts(updated, true, true);
  return finalProd;
}
    ...
    try {
      ...
      if (!error && data && data.length > 0) {
        ...
        if (index !== -1) {
          ...
          saveLocalProducts(products, true, true);
        }
        return updatedProd;
      }
      if (error) {
...
export async function deleteProduct(id) {
  ...
  saveLocalProducts(filtered, true, true);
}
...
export async function getCategories() {
  ...
  if (!fetchedRemote || remoteCategories.length === 0) {
    try {
      const res = await fetch("/api/sync", { cache: "no-store" });
      if (res.ok) {
        const json = await res.json();
        if (json?.store?.categories && Array.isArray(json.store.categories) && json.store.categories.length > 0) {
          const apiCats = json.store.categories.filter(c => c && !["cat-1", "cat-2", "cat-3"].includes(String(c.id)));
          if (apiCats.length > 0) {
            remoteCategories = apiCats;
            fetchedRemote = true;
          }
        }
      }
  ...
  if (fetchedRemote && remoteCategories.length > 0) {
    ...
    saveLocalCategories(merged, false, false);
    return merged;
  }
...
export async function getOffers() {
  ...
  if (!fetchedRemote || remoteOffers.length === 0) {
    try {
      const res = await fetch("/api/sync", { cache: "no-store" });
      if (res.ok) {
        const json = await res.json();
        if (json?.store?.offers && Array.isArray(json.store.offers) && json.store.offers.length > 0) {
