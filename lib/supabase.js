...
export async function deleteOrder(id, authContext = {}) {
  // Strict Authorization check: Only authenticated admin users are allowed to delete orders
  const isAdminUser = authContext?.isAdmin === true || authContext?.user?.email === "admin@devoranaturals.com" || Boolean(getAdminAuthHeader());
  if (!isAdminUser) {
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      ...
      if (existing) {
        ...
        if (typeof window !== "undefined") {
          try {
            const adminSession = getAdminAuthHeader();
            const res = await fetch(`/api/admin/orders/${encodeURIComponent(id)}`, {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
                "x-admin-auth": adminSession || "admin@devoranaturals.com",
              },
            });
...
export async function deleteOffer(id, options = {}) {
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      ...
      if (typeof window !== "undefined") {
        try {
          const adminSession = localStorage.getItem("devora_admin_session") || JSON.stringify({ active: true, email: "admin@devoranaturals.com", ts: Date.now() });
          const res = await fetch(`/api/admin/offers/${encodeURIComponent(id)}?code=${encodeURIComponent(normalizedCode)}`, {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
              "x-admin-auth": adminSession,
            },
          });
          if (res.ok) {
            apiDone = true;
    ...
    try {
      ...
      if (!error && data) {
        return {
          ...
          ...data,
          logo_url:
            local.logo_url !== undefined
              ? local.logo_url
              : (data.logo_url !== undefined ? data.logo_url : ""),
          tagline:
            local.tagline !== undefined
              ? local.tagline
              : (data.tagline !== undefined ? data.tagline : DEFAULT_SETTINGS.tagline),
          store_name:
            local.store_name !== undefined
              ? local.store_name
              : (data.store_name !== undefined ? data.store_name : DEFAULT_SETTINGS.store_name),
          state_shipping_enabled:
            local.state_shipping_enabled !== undefined
          ...
          shipping_charge_tamilnadu:
            local.shipping_charge_tamilnadu !== undefined
              ? local.shipping_charge_tamilnadu
              : (data.shipping_charge_tamilnadu !== undefined ? data.shipping_charge_tamilnadu : 50),
          shipping_charge_other_states:
            local.shipping_charge_other_states !== undefined
              ? local.shipping_charge_other_states
              : (data.shipping_charge_other_states !== undefined ? data.shipping_charge_other_states : 100),
          free_shipping_threshold:
            local.free_shipping_threshold !== undefined
              ? local.free_shipping_threshold
...
export async function updateContactDetails(settingsData) {
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      ...
      // Attempt update with safe columns to prevent PGRST204 errors
      const basePayload = {
        email: settingsData.email || current.email,
        phone: settingsData.phone || current.phone,
        address: settingsData.address || current.address,
        whatsapp: settingsData.whatsapp || current.whatsapp,
      };
...
export async function updateCustomer(id, dataToUpdate) {
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      ...
      // 1. If valid UUID, verify or use directly
      if (isValidUUID(id)) {
        targetId = id;
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      if (isValidUUID(cleanId)) {
        ...
        if (error) console.warn("Supabase deleteCustomer error:", error.message);
      } else {
        // If local id, try deleting by email
        const localCustomers = getLocalCustomers();
        const found = localCustomers.find(c => String(c.id) === cleanId);
        if (found?.email) {
          await supabase.from("customers").delete().ilike("email", found.email.trim().toLowerCase());
        }
      }
    } catch (err) {
      console.warn("Supabase deleteCustomer failed:", err);
    }
  }

  // If we reach here, it either succeeded on Supabase, or Supabase is not configured
...
function saveLocalStorefrontSettings(settings) {
  if (typeof window !== "undefined") {
    try {
      localStorage.setItem(LOCAL_STORAGE_STOREFRONT_KEY, JSON.stringify(settings));
    } catch (e) {
      console.warn("Storage quota warning on saveLocalStorefrontSettings, optimizing storage:", e);
      try {
        localStorage.removeItem("devora_mock_products_v1");
        localStorage.setItem(LOCAL_STORAGE_STOREFRONT_KEY, JSON.stringify(settings));
      } catch (err2) {
        console.error("Failed to save local storefront settings:", err2);
      }
    }
  }
}
    ...
    try {
      ...
      if (!error && data) {
        const merged = {
          ...
          ...local,
          ...data,
          announcements: Array.isArray(local.announcements) ? local.announcements : DEFAULT_STOREFRONT_SETTINGS.announcements,
          hero_cards: Array.isArray(local.hero_cards) ? local.hero_cards : DEFAULT_STOREFRONT_SETTINGS.hero_cards,
          value_props: Array.isArray(local.value_props) ? local.value_props : DEFAULT_STOREFRONT_SETTINGS.value_props,
          promos_list: Array.isArray(local.promos_list) ? local.promos_list : DEFAULT_STOREFRONT_SETTINGS.promos_list,
          testimonials: Array.isArray(local.testimonials) ? local.testimonials : DEFAULT_STOREFRONT_SETTINGS.testimonials,
          faqs: Array.isArray(local.faqs) ? local.faqs : DEFAULT_STOREFRONT_SETTINGS.faqs,
          spotlight_enabled: local.spotlight_enabled !== undefined ? local.spotlight_enabled : DEFAULT_STOREFRONT_SETTINGS.spotlight_enabled,
          spotlight_badge: local.spotlight_badge || DEFAULT_STOREFRONT_SETTINGS.spotlight_badge,
          spotlight_image: local.spotlight_image || DEFAULT_STOREFRONT_SETTINGS.spotlight_image,
          spotlight_title: local.spotlight_title || DEFAULT_STOREFRONT_SETTINGS.spotlight_title,
          spotlight_description: local.spotlight_description || DEFAULT_STOREFRONT_SETTINGS.spotlight_description,
          spotlight_stat_1_val: local.spotlight_stat_1_val || DEFAULT_STOREFRONT_SETTINGS.spotlight_stat_1_val,
          spotlight_stat_1_lbl: local.spotlight_stat_1_lbl || DEFAULT_STOREFRONT_SETTINGS.spotlight_stat_1_lbl,
          spotlight_stat_2_val: local.spotlight_stat_2_val || DEFAULT_STOREFRONT_SETTINGS.spotlight_stat_2_val,
          spotlight_stat_2_lbl: local.spotlight_stat_2_lbl || DEFAULT_STOREFRONT_SETTINGS.spotlight_stat_2_lbl,
          spotlight_stat_3_val: local.spotlight_stat_3_val || DEFAULT_STOREFRONT_SETTINGS.spotlight_stat_3_val,
          spotlight_stat_3_lbl: local.spotlight_stat_3_lbl || DEFAULT_STOREFRONT_SETTINGS.spotlight_stat_3_lbl,
          spotlight_stat_4_val: local.spotlight_stat_4_val || DEFAULT_STOREFRONT_SETTINGS.spotlight_stat_4_val,
          spotlight_stat_4_lbl: local.spotlight_stat_4_lbl || DEFAULT_STOREFRONT_SETTINGS.spotlight_stat_4_lbl,
          promoBannerBadge: local.promoBannerBadge || DEFAULT_STOREFRONT_SETTINGS.promoBannerBadge,
          promoBannerLink: local.promoBannerLink || DEFAULT_STOREFRONT_SETTINGS.promoBannerLink,
          promoBannerBtnText: local.promoBannerBtnText || DEFAULT_STOREFRONT_SETTINGS.promoBannerBtnText,
          navbar: local.navbar || DEFAULT_STOREFRONT_SETTINGS.navbar,
          footer: local.footer || DEFAULT_STOREFRONT_SETTINGS.footer,
        };
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      ...
      for (const col of SUPABASE_STOREFRONT_COLUMNS) {
        if (settingsData[col] !== undefined) {
          supabasePayload[col] = settingsData[col];
        }
      }

      const { data: existing } = await supabase.from("storefront_settings").select("*").limit(1).maybeSingle();
      if (existing) {
        await supabase
          .from("storefront_settings")
          .update(supabasePayload)
