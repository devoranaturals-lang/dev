...
export async function getProducts() {
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      ...
      if (!error && data) {
        remoteProducts = data.filter((p) => !isDemoProduct(p) && !deletedIds.includes(String(p.id)));
        fetchedRemote = true;
      }
    } catch (err) {
...
export async function getProductById(id) {
  ...
  return products.find((p) => !isDemoProduct(p) && (String(p.id) === String(id) || String(p.slug) === String(id))) || null;
}
