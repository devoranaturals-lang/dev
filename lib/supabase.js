  ...
  if (isSupabaseConfigured) {
    try {
      ...
      const adminSession = typeof window !== "undefined"
        ? localStorage.getItem("devora_admin_session")
        : null;

      if (targetId) {
        // Use server-side admin API route to bypass RLS
        const res = await fetch(`/api/admin/customers/${encodeURIComponent(targetId)}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            ...(adminSession ? { "x-admin-auth": adminSession } : {}),
          },
        ...
        if (res.ok) {
          ...
          if (json.customer) {
            supabaseResult = json.customer;
          }
        } else {
          console.warn("Admin API updateCustomer error:", res.status);
        }
      } else if (supabase) {
